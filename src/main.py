...
from typing import Final, Iterator, Dict, Optional
...
def parse_price_from_soup(soup: BeautifulSoup) -> Optional[float]:
    try:
        price = (
            soup.find(id="tabViewForm:tabView:0:test").encode_contents().decode("UTF-8")
        )
...
if __name__ == "__main__":
    ...
    for isin in get_urls(args.isin_file):
        isin = isin.strip()
        url = (
            "https://www.lyxoretf.de/de/retail/products/equity-etf/lyxor-new-energy-ucits-etf-dist/%s/eur"
            % isin
        )
